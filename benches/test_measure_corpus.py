...
class MetricsTest(unittest.TestCase):
    ...
    def test_corpus_latency_and_interval_are_per_frame(self):
        ...
        metric = module.metric(benchmark, self.estimate(), "compiler=test")
    ...
    def test_missing_element_count_is_rejected(self):
        ...
        with self.assertRaises(ValueError):
            module.metric(benchmark, self.estimate(), "test")
    ...
    def test_comparison_implementations_have_same_chart_name(self):
        for implementation in ("rust", "libmbus"):
            ...
            metric = module.metric(benchmark, self.estimate(), "test")
    ...
    def test_missing_results_and_invalid_timings_fail(self):
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(ValueError):
                module.collect(Path(directory), "test")
        for value in (0, -1, float("nan"), float("inf")):
            with self.assertRaises(ValueError):
                module.metric({"full_id": "comparison/decode/rust", "throughput": {"Elements": 73}}, self.estimate(value), "test")
